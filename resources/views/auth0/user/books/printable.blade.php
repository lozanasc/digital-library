...
<html>
  <head>
    <title>Request | {{$request_info->book_name}}</title>
    <meta charset="utf-8">
  ...
  <body>
    <div class="container">
      <div class="bg-light p-5">
        ...
        <h1 class="text-center m-0">{{$request_info->request_name}}</h1>
        <div class="row pt-3 mb-2">
          ...
          <div class="col-md-6 text-right">
            ...
            <p class="text-muted mb-0"><i>Date requested: {{$request_info->created_at}}</i></p>
          </div>
        </div>
        <div class="row b-t pt-5">
          <div class="col-md-6 pt-3 center">
            <h5>Request Information</h5>
            <p><strong>Return Date:</strong> {{$request_info->return_date}}
            <br>
              <strong>Reason:</strong> {{$request_info->reason}}
            </p>
            <h5>User Information</h5>
            <p><strong>Contact:</strong> {{$user_info->contact}}
            <br>
          ...
          <div class="col-md-6 text-right">
            <h5>Book Details</h5>
            <p>ID: {{$requested_book->id}}</p>
            <p>TITLE: {{$requested_book->name}}</p>
            <p>AUTHOR: {{$requested_book->author}}</p>
            <p>DATE PUBLISHED: {{$requested_book->date_published}}</p>
          </div>
        </div>
        <div class="table-responsive">
          <table class="table text-nowrap">
            ...
            <tr>
              <td>
                <img src="{{ Storage::url($requested_book->book_cover) }}" alt="Cover of the book" height="160" width="600">
              </td>
              <td>{{$requested_book->isbn_no}}</td>
              <td>{{$request_info->status}}</td>
              <td>{{$request_info->return_date}}</td>
            </tr>
          </table>
        </div>
      </div>
